...
export class CompositionState {
    constructor() {
        ...
        // ====================================================================
        // BASS DATA BY CHORD ID - Chord-ID-Based Bass Storage (Phase 2)
        // ====================================================================
        // Maps chord ID -> { notes: [...], userEdited: boolean }
        // This replaces position-based bass tracking. When chords are reordered,
        // their bass data follows them because it's keyed by unique chord ID.
        // This enables:
        // - Bass edits surviving chord reordering
        // - Duplicate chords having independent bass data
        // - Clean separation of bass data from array position
    ...
    checkIfBassIsEdited(chordIndex) {
        // First check the new chord ID-based Map (most reliable)
        const progressionData = this.exportToProgressionData();
        const chord = progressionData[chordIndex];
        if (chord?.id && this.bassDataByChordId) {
            const bassData = this.bassDataByChordId.get(chord.id);
            if (bassData && bassData.userEdited) {
                return true;
            }
        }

        // Also check the BuildingBlock's userEdited flag (legacy system)
        const block = this.bassBlockSequence?.blocks?.[chordIndex];
        if (block && block.userEdited) {
            return true;
        }

        // Fallback: Check all measures belonging to this chord
    ...
    syncMeasuresToBuildingBlocks() {
        ...
        for (const [chordIndex, allNotes] of chordNotes) {
            ...
            // EXCEPTION: If the block is user-edited, trust the note's actual duration
            for (const note of allNotes) {
                const startUnit = Math.round(note.beat * UNITS_PER_BEAT);

                // Calculate the note's actual duration from its duration property
                const baseDuration = note.duration || '4n';
                let noteDurationUnits = durationToUnits(baseDuration);
                if (note.dotted && !baseDuration.includes('.')) {
                    noteDurationUnits = Math.round(noteDurationUnits * 1.5);
                }
                ...
                // BUT: If the block is user-edited, trust the note's actual duration
                // This preserves user duration changes (e.g., whole note → half note)
                const useFullBlockDuration = allNotes.length === 1 && startUnit === 0 && !block.userEdited;

                let durationUnits;
                if (useFullBlockDuration) {
                    durationUnits = totalUnits;
                } else {
                    durationUnits = noteDurationUnits;
                }
    ...
    initializeTrebleBlockSequence() {
        ...
        // Calculate total beats - prefer chord segment total over measure count
        // This is critical when chord durations don't evenly fill measures
        // (e.g., 4 chords × 2 beats = 8 beats in 6/8 time = 2.67 measures)
        const beatsPerMeasure = getBeatsPerMeasureFromTimeSignature(timeSignature);
        let totalBeats;
    ...
    insertBassNoteWithShift(chordIndex, unitPosition, durationUnits, pitches, options = {}) {
        ...
        block.userEdited = true;  // Prevents automatic regeneration when chord properties change
    ...
    resetBassBlockToDefault(chordIndex) {
        ...
        // Clear userEdited flags FIRST (before regeneration)
        // This ensures the regeneration treats this as a fresh auto-generation
        block.autoGenerated = true;
        block.userEdited = false;

        // Clear from bassDataByChordId Map (new chord ID-based system)
        ...
        // This uses the same bass pattern generation as initial chord creation
        this.regenerateAutoBassByChordIndex(chordIndex);

        // Re-render to measures
        this.renderBassBlocksToMeasures();

        // Clear the edited flag on the measure(s) for this chord
        const blockInfo = this.getBassBlockByIndex(chordIndex);
        if (blockInfo) {
            const beatsPerMeasure = getBeatsPerMeasureFromTimeSignature(this.metadata.timeSignature);
            const startMeasure = Math.floor(blockInfo.blockStartBeat / beatsPerMeasure);
            const endMeasure = Math.floor((blockInfo.blockStartBeat + blockInfo.beats - 0.01) / beatsPerMeasure);
            for (let i = startMeasure; i <= endMeasure && i < this.measures.length; i++) {
                if (this.measures[i]) {
                    this.measures[i].notation.bass.autoGenerated = true;
                }
            }
        }
    ...
    regenerateAllAutoBassByBuildingBlock() {
        ...
        // Process each building block and UPDATE THE BLOCK with generated notes
        // This is the source of truth - renderBassBlocksToMeasures() will render them
        for (let blockIndex = 0; blockIndex < this.bassBlockSequence.blocks.length; blockIndex++) {
            const block = this.bassBlockSequence.blocks[blockIndex];
            if (!block) continue;

            const chord = block.chord;
            const durationBeats = block.beats;
            ...
            const chordBassPattern = this.storedProgressionData?.[blockIndex]?.bassPattern;
            ...
            // Clear the block and set new notes
            // Each note from generateBuildingBlockBass has: pitches, beat, duration, durationBeats
            // Use _initializeUnits with empty pitches to clear all existing notes
            block._initializeUnits([]);

            for (const note of blockBassNotes) {
                const startUnit = note.beat * UNITS_PER_BEAT;
                const durationUnits = (note.durationBeats || 2) * UNITS_PER_BEAT; // Default 2 beats if not specified
                block.setNote(startUnit, durationUnits, note.pitches, {
                    autoGenerated: true
                });
            }
        ...
        // Now render the updated building blocks to measures
        this.renderBassBlocksToMeasures();
    ...
    regenerateAutoBassByChordIndex(chordIndex) {
        ...
        // Sync block.chord from storedProgressionData to ensure omittedNotes and other properties are current
        // This is needed because updateChord() updates measure.chord but not block.chord
        if (this.storedProgressionData && this.storedProgressionData[chordIndex]) {
            block.chord = {
                ...block.chord,
                ...this.storedProgressionData[chordIndex]
            };
        }
        ...
        const previousBlock = chordIndex > 0 ? this.bassBlockSequence.blocks[chordIndex - 1] : null;
        const previousChord = previousBlock ? previousBlock.chord : null;
     ...
     * Updates the building block (source of truth) and then renders to measures
    ...
    placeChordVoicingInBassForChord(chordIndex) {
        ...
        // Clear the block and set a single note spanning the full duration
        block._initializeUnits([]);

        // Set a single note spanning the entire block
        const durationUnits = block.beats * UNITS_PER_BEAT;
        block.setNote(0, durationUnits, bassNotes, {
            autoGenerated: false  // User explicitly reverted to chord voicing
        });
    ...
    setTimeSignature(num, denom) {
        ...
        if (hasTrebleMultiVoice && hasTrebleNotes) {
            ...
            // NOTE: Treble block reinitialization moved to AFTER bass is rendered
            // because initializeTrebleBlockSequence() depends on measures.length,
            // which is updated by renderBassBlocksToMeasures()
        }
        ...
        // 6. Reinitialize empty treble block AFTER bass is rendered
        // This is needed when there are no treble notes, because initializeTrebleBlockSequence()
    ...
    scaleChordDurations(scaleFactor) {
        ...
        // CRITICAL: Also update storedProgressionData with scaled beats
        // Without this, buildChordSegments() would use the OLD unscaled durations
        // because exportToProgressionData() returns storedProgressionData
        if (this.storedProgressionData && this.storedProgressionData.length > 0) {
            progressionData.forEach((chord, index) => {
                if (this.storedProgressionData[index]) {
                    this.storedProgressionData[index].beats = chord.beats;
                }
            });
        }
        ...
        progressionData.forEach((chordData, chordIndex) => {
            ...
            while (remainingBeats > 0) {
                ...
                const measureIndex = this.addMeasure({
                    chord: {
                        id: chordData.id, // CRITICAL: Chord ID for bass preservation
                        root: chordData.root,
                        type: chordData.type,
                        ...
                        // Preserve omitted notes for proper bass generation and playback
    ...
    syncWithProgressionData(progressionData, options = {}) {
        ...
        // ================================================================
        // Save existing bass block data BEFORE any reinitialization
        ...
        const existingBlockData = this.bassBlockSequence.blocks.map((block, idx) => {
            ...
            return {
                ...
                userEdited: block.userEdited || false,
                chordRoot: block.chord?.root,
                chordType: block.chord?.type, // Also track type for disambiguation
        ...
        if (needsReinitialize) {
            ...
            // CRITICAL: Restore user-edited bass blocks from backup
            // Match by chord root to handle insertion/reorder at any position
            ...
            for (let i = 0; i < progressionData.length; i++) {
                ...
                // Find matching backup data by chord root
                ...
                if (matchingBackup && matchingBackup.userEdited) {
                    ...
                    if (matchingBackup.units && matchingBackup.units.length > 0) {
                        // Adjust units array length to match new duration
                        const totalUnits = block.beats * UNITS_PER_BEAT;
                        block.units = [];
                        for (let j = 0; j < totalUnits; j++) {
                            if (j < matchingBackup.units.length) {
                                block.units.push(matchingBackup.units[j].clone());
                            } else {
                                // Extend with the last unit's pitches
                                const lastPitches = matchingBackup.units[matchingBackup.units.length - 1]?.pitches || [];
                                block.units.push(new Unit({ pitches: [...lastPitches], parentIndex: 0 }));
                            }
                        }
                    }
                    // Remove from backup so we don't match it again (for duplicates)
                    const backupIdx = existingBlockData.indexOf(matchingBackup);
                    if (backupIdx >= 0) {
                        existingBlockData.splice(backupIdx, 1);
                    }
                } else if (chord.lhNotes && chord.lhNotes.length > 0) {
            ...
            progressionData.forEach((chord, i) => {
                ...
                if (block) {
                    ...
                    const chordPropsChanged = oldType !== newType || oldInversion !== newInversion || oldOctaveShift !== newOctaveShift || notesChanged;
                    ...
                    if (block.userEdited && chordPropsChanged) {
                        ...
                        // Skip bass note regeneration - user's edits are preserved
                    } else if (chordPropsChanged) {
                        ...
                        // Apply omittedNotes filter (same as initializeBassBlockSequence)
                        const omittedNotes = chord.omittedNotes || [];
                        if (omittedNotes.length > 0 && bassNotes) {
                            bassNotes = bassNotes.filter(n => !omittedNotes.includes(n));
                        }
    ...
    exportToProgressionData() {
        ...
        this.measures.forEach((measure, idx) => {
            ...
            if (chordIndex !== undefined && !seenChordIndices.has(chordIndex)) {
                ...
                const chordData = {
                    id: measure.chord.id || `chord_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`, // Unique ID for bass preservation
    ...
    /**
     * Get the tempo (BPM)
     * @returns {number} The tempo in beats per minute (defaults to 120)
     */
    getTempo() {
        return this.metadata.tempo || 120;
    }

    /**
     * Set the tempo (BPM)
     * @param {number} tempo - The tempo in beats per minute (40-300)
     */
    setTempo(tempo) {
        const validTempo = Math.max(40, Math.min(300, parseInt(tempo) || 120));
        this.metadata.tempo = validTempo;
        this.events.emit('metadataChanged', { field: 'tempo', value: validTempo });
        this.events.emit('tempoChanged', validTempo);
    }
    ...
    /**
     * Check if reducing chord duration would truncate user-edited bass notes
     * @param {number} chordIndex - The chord index to check
    ...
    checkDurationTruncation(chordIndex, newBeats) {
        ...
        // Add chord name for the warning dialog
        const chord = this.getChord(chordIndex);
        const chordName = chord ? `${chord.root} ${chord.type}` : `Chord ${chordIndex + 1}`;
        ...
        return {
            ...
            truncationInfo: {
                ...truncationInfo,
                chordName,
                oldDuration: block.beats,
                newDuration: newBeats
            }
        };
    }
     ...
     * Returns truncation info if user confirmation is needed before proceeding.
     * @param {number} chordIndex - The chord index to update
     * @param {number} newBeats - New duration in beats
     * @returns {Object|boolean} - { needsConfirmation, truncationInfo } if warning needed, true on success, false on failure
     */
    updateChordDuration(chordIndex, newBeats) {
        ...
        // Check if truncation warning is needed
        const truncationCheck = this.checkDurationTruncation(chordIndex, newBeats);
        if (truncationCheck) {
            // Return the truncation info - caller should show warning and call forceApplyChordDuration if user confirms
            return truncationCheck;
        }

        // No warning needed - apply the change directly
        return this._applyChordDuration(chordIndex, newBeats, progressionData);
    }

    /**
     * Internal method to actually apply the chord duration change
     * @private
     */
    _applyChordDuration(chordIndex, newBeats, progressionData = null) {
        if (!progressionData) {
            progressionData = this.exportToProgressionData();
        }
    ...
    forceApplyChordDuration(chordIndex, newBeats) {
        // Ensure blocks are synced first
        if (this.bassBlockSequence.blocks.length > 0) {
            this.syncMeasuresToBuildingBlocks();
        }
        return this._applyChordDuration(chordIndex, newBeats);
    }
    ...
    // BASS DATA BY CHORD ID - Chord-ID-Based Bass Storage (Phase 2)
    // ========================================================================
    // These methods manage bass data keyed by unique chord IDs.
    // This enables bass edits to survive chord reordering/insertion/removal.

    /**
     * Get bass data for a chord by its unique ID
     * @param {string} chordId - The unique chord ID
     * @returns {Object|null} - Bass data object { notes: [...], userEdited: boolean } or null
     */
    getBassDataForChord(chordId) {
        return this.bassDataByChordId.get(chordId) || null;
    }

    /**
     * Set bass data for a chord by its unique ID
     * @param {string} chordId - The unique chord ID
     * @param {Object} data - Bass data { notes: [...], userEdited: boolean }
     */
    setBassDataForChord(chordId, data) {
        if (!chordId) {
            console.warn('[setBassDataForChord] Cannot set bass data without chord ID');
            return;
        }
        this.bassDataByChordId.set(chordId, {
            notes: data.notes || [],
    ...
    /**
     * Clear bass data for a chord by its unique ID
     * @param {string} chordId - The unique chord ID to remove
     */
    ...
    /**
     * Check if a chord has user-edited bass data
     * @param {string} chordId - The unique chord ID
     * @returns {boolean} - True if chord has user-edited bass data
     */
    hasUserEditedBass(chordId) {
        const data = this.bassDataByChordId.get(chordId);
        return data?.userEdited === true;
    }

    /**
     * Get all chord IDs that have user-edited bass data
     * @returns {string[]} - Array of chord IDs with user edits
     */
    getChordIdsWithEditedBass() {
        const ids = [];
        for (const [chordId, data] of this.bassDataByChordId) {
            if (data.userEdited) {
                ids.push(chordId);
            }
        }
    ...
    /**
     * Restore bass data from chord IDs after structural changes (reorder/insert/remove)
     * This is the core of the bass preservation system - it restores user-edited bass
     * to the correct positions based on chord IDs, regardless of where chords moved.
     *
     * Algorithm:
     * 1. Get the current progression data (with chord IDs)
     * 2. For each chord with user-edited bass data in the Map:
     *    a. Find its NEW position in the progression by chord ID
     *    b. Find the measures that now contain this chord
     *    c. Place the stored bass notes in those measures
    ...
    restoreBassFromChordIds() {
        ...
        const chordPositions = new Map(); // chordId -> { chordIndex, startBeat, beats, measures: [measureIndices] }
        let absoluteBeat = 0;
        ...
        progressionData.forEach((chord, chordIndex) => {
            ...
            // Find which measures this chord spans
            const startMeasure = Math.floor(startBeat / beatsPerMeasure);
            const endBeat = startBeat + chordBeats;
            const endMeasure = Math.ceil(endBeat / beatsPerMeasure) - 1;
            const measures = [];
            ...
            absoluteBeat += chordBeats;
        });

        // Now restore bass data for chords that have user edits
        for (const [chordId, bassData] of this.bassDataByChordId) {
            if (!bassData.userEdited) {
                continue;
            }
    ...
    /**
     * Helper: Place bass notes for a chord into the appropriate measures
     * Handles splitting notes across measure boundaries with ties
     * @param {Array} notes - The bass notes to place (with relative beat positions within chord)
     * @param {Object} positionInfo - { chordIndex, startBeat, beats, measures, chord }
    ...
    _placeBassNotesForChord(notes, positionInfo, beatsPerMeasure) {
        ...
        for (const measureIndex of measures) {
            if (measureIndex < this.measures.length) {
                const measure = this.measures[measureIndex];
                if (measure.notation?.bass?.voices?.[0]) {
                    // Only clear notes that belong to this chord
                    measure.notation.bass.voices[0].notes =
                        (measure.notation.bass.voices[0].notes || []).filter(n =>
                            n.chordIndex !== chordIndex
                        );
                }
            }
        }

        // Place each note, splitting across measure boundaries if needed
        for (const note of notes) {
            // note.beat is relative to chord start
            const absoluteNoteBeat = startBeat + (note.beat || 0);
            const noteBeats = durationToBeatsCanonical(note.duration, note.dotted);

            // Find which measure this note starts in
            const noteMeasureIndex = Math.floor(absoluteNoteBeat / beatsPerMeasure);
            if (noteMeasureIndex >= this.measures.length) continue;

            const beatInMeasure = absoluteNoteBeat - (noteMeasureIndex * beatsPerMeasure);
            const measureEndBeat = beatsPerMeasure;
            const noteEndBeat = beatInMeasure + noteBeats;

            if (noteEndBeat <= measureEndBeat) {
                // Note fits entirely in one measure
                const newNote = {
                    ...JSON.parse(JSON.stringify(note)),
                    beat: beatInMeasure,
                    chordIndex: chordIndex
                };
                this._addBassNoteToMeasure(noteMeasureIndex, newNote);
            } else {
                // Note crosses measure boundary - split it with ties
                const firstPartBeats = measureEndBeat - beatInMeasure;
                const remainingBeats = noteBeats - firstPartBeats;

                // First part (in current measure, ties forward)
                const firstPart = {
                    ...JSON.parse(JSON.stringify(note)),
                    beat: beatInMeasure,
                    chordIndex: chordIndex,
                    tied: true, // Ties forward to next part
                    isTied: false
                };
                const firstDuration = beatsToDurationCanonical(firstPartBeats);
                firstPart.duration = firstDuration.duration;
                firstPart.dotted = firstDuration.dotted;
                this._addBassNoteToMeasure(noteMeasureIndex, firstPart);

                // Second part (in next measure, tied from first)
                if (noteMeasureIndex + 1 < this.measures.length && remainingBeats > 0) {
                    const secondPart = {
                        ...JSON.parse(JSON.stringify(note)),
                        beat: 0, // Starts at beginning of next measure
                        chordIndex: chordIndex,
                        tied: false,
                        isTied: true // Tied from previous part
                    };
                    const secondDuration = beatsToDurationCanonical(remainingBeats);
                    secondPart.duration = secondDuration.duration;
                    secondPart.dotted = secondDuration.dotted;
                    this._addBassNoteToMeasure(noteMeasureIndex + 1, secondPart);
                }
            }
        }
    }
    ...
    /**
     * Helper: Add a bass note to a specific measure
     * @param {number} measureIndex - The measure index
     * @param {Object} note - The note to add
     */
    _addBassNoteToMeasure(measureIndex, note) {
        if (measureIndex >= this.measures.length) return;

        const measure = this.measures[measureIndex];
        if (!measure.notation?.bass?.voices?.[0]) {
            if (!measure.notation) measure.notation = { treble: { voices: [{ notes: [] }] }, bass: { voices: [{ notes: [] }] } };
            if (!measure.notation.bass) measure.notation.bass = { voices: [{ notes: [] }] };
            if (!measure.notation.bass.voices) measure.notation.bass.voices = [{ notes: [] }];
            if (!measure.notation.bass.voices[0]) measure.notation.bass.voices[0] = { notes: [] };
        }

        measure.notation.bass.voices[0].notes.push(note);
    ...
    /**
     * Helper: Clear all bass notes for a chord's position in measures
     * Used when a chord has userEdited=true but empty notes (intentional rest/silence)
     * @param {Object} positionInfo - Position info with startBeat, measures array
     * @param {number} beatsPerMeasure - Beats per measure for calculation
     */
    _clearBassNotesForChord(positionInfo, beatsPerMeasure) {
        const { startBeat, beats: chordBeats, measures: chordMeasures } = positionInfo;

        const chordStartBeat = startBeat;
        const chordEndBeat = startBeat + chordBeats;

        // For each measure this chord spans, remove bass notes in the chord's beat range
        for (const measureIndex of chordMeasures) {
            ...
            // Filter out notes that belong to this chord (based on absolute beat position)
            measure.notation.bass.voices[0].notes = measure.notation.bass.voices[0].notes.filter(note => {
                const noteAbsoluteBeat = measureStartBeat + (note.beat || 0);
                // Keep notes that are outside this chord's beat range
                return noteAbsoluteBeat < chordStartBeat || noteAbsoluteBeat >= chordEndBeat;
            });
        }
    ...
    reorderChord(fromIndex, toIndex) {
        ...
        this.syncWithProgressionData(progressionData, {
            key: this.metadata.key,
            timeSignature: this.metadata.timeSignature || DEFAULT_TIME_SIGNATURE,
            skipAutoGenerateBass: true  // Preserve existing bass notes in reordered blocks
        });
    ...
    insertChord(atIndex, chordData) {
        ...
        clonedBassBlocks.forEach(({ newIndex, clonedBlock }) => {
            if (newIndex < this.bassBlockSequence.blocks.length) {
                ...
                if (targetBlock && clonedBlock.units) {
                    ...
                    // CRITICAL: Preserve userEdited flag so duration changes aren't lost
                    targetBlock.userEdited = clonedBlock.userEdited || false;
                }
            }
        });

        // Generate bass for the NEW chord only (if auto-generate is enabled)
        // ONLY for non-user-edited chords (new chords at atIndex)
        if (this.settings.autoGenerateBass) {
        ...
        // BASS PRESERVATION: Restore user-edited bass from chord ID map
        // This ensures existing user edits follow their chords to new positions
        if (this.bassDataByChordId.size > 0) {
            this.restoreBassFromChordIds();
        }
    ...
    removeChord(atIndex) {
        ...
        // BASS PRESERVATION: Get the chord ID before removing so we can clean up the Map
        const removedChordId = progressionData[atIndex]?.id;
        ...
        // BASS PRESERVATION: Clean up bass data for the removed chord
        if (removedChordId) {
            this.clearBassDataForChord(removedChordId);
        }
        ...
        // BASS PRESERVATION: Restore user-edited bass from chord ID map
        // This ensures remaining user edits follow their chords to new positions
        if (this.bassDataByChordId.size > 0) {
            this.restoreBassFromChordIds();
        }
    ...
    saveEditedBassNotesForMeasure(measureIndex) {
        ...
        for (let i = 0; i < this.measures.length; i++) {
            ...
            // First by chord ID (preferred), then by chordIndex (fallback)
            let belongsToChord = false;

            if (chordId && measure.chord?.id === chordId) {
                belongsToChord = true;
            } else if (measure.chord?.chordIndex === chordIndex) {
                belongsToChord = true;
            } else {
        ...
        // ================================================================
        // NEW: Store bass data by chord ID for bass preservation system
        // ================================================================
        // Collect all bass notes for this chord across all its measures
        if (chordId) {
            const allBassNotesForChord = [];
            for (const idx of measuresToSave) {
                const measure = this.measures[idx];
                const notes = measure.notation?.bass?.voices?.[0]?.notes || [];
                // Deep copy notes to preserve all properties
                notes.forEach(note => {
                    allBassNotesForChord.push(JSON.parse(JSON.stringify(note)));
                });
            }
            // Store in the bassDataByChordId Map
            this.setBassDataForChord(chordId, {
        ...
        clonedBassBlocks.forEach((data, i) => {
            ...
            if (newChordIdx < this.bassBlockSequence.blocks.length) {
                ...
                if (newBlock && clonedBlock.units) {
                    ...
                    // CRITICAL: Preserve userEdited flag so duration changes aren't lost
                    newBlock.userEdited = clonedBlock.userEdited || false;
                }
            }
            ...
            if (origChordIdx < this.bassBlockSequence.blocks.length) {
                ...
                if (origBlock && clonedBlock.units) {
                    ...
                    // CRITICAL: Preserve userEdited flag so duration changes aren't lost
                    origBlock.userEdited = clonedBlock.userEdited || false;
                }
            }
        });
