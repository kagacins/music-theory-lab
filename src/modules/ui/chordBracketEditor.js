        ...
        <div class="chord-bracket-editor bg-white border-2 border-indigo-500 rounded-lg shadow-xl overflow-hidden" style="width: 400px;">
            ...
            <div class="tab-content-area bg-white">
                ...
                <div class="tab-content edit-controls p-3 text-xs" data-tab-content="edit" style="display: ${activeTab === 'edit' ? 'block' : 'none'}">
                    ...
                    <!-- Row 4: Delete Chord button -->
                    <button class="delete-chord-btn w-full px-3 py-2 mt-2 text-[11px] font-semibold bg-red-500 hover:bg-red-600 text-white rounded-md flex items-center justify-center gap-2 transition" title="Remove this chord from the progression">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function attachEditorEventListeners(editor, chordIndex, key) {
    ...
    // Delete Chord button - removes chord from progression
    const deleteChordBtn = editor.querySelector('.delete-chord-btn');
    if (deleteChordBtn) {
        deleteChordBtn.addEventListener('click', () => {
            // Close the editor first
            hideChordBracketEditor();

            // Remove the chord using the same function as the X button on chord cards
            if (window.removeChordFromProgression) {
                window.removeChordFromProgression(chordIndex);
            }
        });
    }
