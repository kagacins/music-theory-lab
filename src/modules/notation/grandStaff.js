...
export function renderGrandStaffMeasure(context, measureData, options = {}) {
  ...
  if (measureData.metadata && measureData.metadata.chordSymbol && isBlockStart) {
    ...
    // Calculate position based on beat offset within measure
    // If chord starts mid-measure, position the symbol at that beat position
    const beatsPerMeasureForPosition = getBeatsPerMeasureFromTimeSignature(timeSignature);
    const beatFraction = chordBeatOffset / beatsPerMeasureForPosition;
...
export function renderGrandStaffSystem(container, measures, options = {}) {
  ...
  const chordStartBeatInMeasure = new Map(); // measureIndex -> beat offset
  ...
  for (let i = 0; i < measures.length; i++) {
    ...
    const result = renderGrandStaffMeasure(context, measures[i], {
      ...
      // Chord positioning info for mid-measure chord starts
