...
export class NotationComposer {
   ...
   */
  syncFromProgression() {
    // Note: isSyncingFromProgression is already set by window.syncNotationFromProgression
    ...
    // Note: Removed measureManager preservation logic - compositionState is now single source of truth
    ...
    for (let i = 0; i < progressionData.length; i++) {
      ...
      if (this.compositionState) {
        ...
        // IMPORTANT: Preserve metadata (especially chord symbols) when updating
        const existingMeasure = this.compositionState.getMeasure(i);
        const preservedMetadata = existingMeasure?.metadata ? { ...existingMeasure.metadata } : {};
        ...
        // Restore the preserved metadata after updateChord
        if (existingMeasure && Object.keys(preservedMetadata).length > 0) {
          existingMeasure.metadata = preservedMetadata;
        }
      ...
      if (this.compositionState && this.compositionState.getMeasureCount() > i) {
        ...
        if (stateMeasure && stateMeasure.notation && stateMeasure.notation.bass) {
          ...
          if (bassVoices && bassVoices[0] && bassVoices[0].notes && bassVoices[0].notes.length > 0) {
            // Use bass notes from compositionState (include rests)
            console.log(`[composerIntegration] Measure ${i} bass notes from state:`, bassVoices[0].notes.map(n => ({ beat: n.beat, isTied: n.isTied })));
            measureData.bassNotes = bassVoices[0].notes
              .map(note => {
                const mapped = {
                  ...
                  dotted: note.dotted || false,
                  tie: note.tie,
                  isTied: note.isTied  // For cross-measure ties
                };
                console.log(`[composerIntegration] Mapped note: isTied=${note.isTied} -> ${mapped.isTied}`);
                return mapped;
              });
      ...
      // Note: Removed measureManager.addMeasure() - compositionState handles all measure data now
    }

    // CRITICAL: Call render with bypass flag while sync is still in progress
    // This allows the final render to execute, then we reset the flag after
    this.render(true); // Bypass sync check for final render

    // CRITICAL: Delay resetting the flag to ensure any event-driven renders triggered by
    // the render above are still blocked. Without this delay, renders can slip through
    // between sync cycles and catch the state with only 1 measure built.
    setTimeout(() => {
      this.isSyncingFromProgression = false;
    }, 50);
    } finally {
      // Ensure flag is eventually reset even if there's an error
      setTimeout(() => {
        this.isSyncingFromProgression = false;
      }, 100);
    }
  ...
  performRender() {
    ...
    const measures = hasCompositionState
      ? this.compositionState.measures.map(m => ({
          ...
          metadata: m.metadata,  // CRITICAL: Include metadata for chord symbols and other measure-level data
        }))
