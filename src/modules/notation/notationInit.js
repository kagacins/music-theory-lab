...
export function initEnhancedNotation(options = {}) {
  ...
  noteEditor = new NoteEditor({
    ...
    onNoteDelete: (deletion) => {
      ...
      // 1. Delete via compositionState API so autoGenerated flags stay accurate
      if (notationComposer.compositionState &&
          typeof notationComposer.compositionState.removeNote === 'function') {
      ...
      } else if (notationComposer.compositionState) {
        // Fallback: direct mutation (legacy) – keep for backward compatibility
        ...
        if (measure) {
          ...
          if (notes && deletion.noteIndex < notes.length) {
            ...
            console.log('[NotationInit] Notes after delete (fallback):', notes);
          }
        }
      }
