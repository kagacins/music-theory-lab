...
export class NoteEditor {
  ...
  findNoteAtPosition(x, y) {
    ...
    // Debug: Log click position and auto-generated rest regions
    const autoGenRests = this.noteRegions.filter(r => r.isAutoGenerated);
    if (autoGenRests.length > 0) {
      console.log(`[findNoteAtPosition] Click at layout (${layoutX.toFixed(1)}, ${layoutY.toFixed(1)}), auto-gen rests:`,
        autoGenRests.map(r => ({ beat: r.beat, bounds: r.bounds, noteHeadPositions: r.noteHeadPositions })));
    }
    ...
    if (noteHeadMatches.length > 0) {
      ...
      console.log(`[findNoteAtPosition] Returning from note head match:`, {
    ...
    console.log(`[findNoteAtPosition] Returning from bounding box match:`, {
      noteIndex: result.noteIndex,
      beat: result.beat,
      isRest: result.isRest,
      isAutoGenerated: result.isAutoGenerated,
      ...
      if (window.getCompositionState) {
        ...
        if (compositionState && compositionState.measures[measureIndex]) {
          ...
          if (voice && voice.notes[noteIndex]) {
            ...
            changedCount++;
            console.log('[NoteEditor] ✅ Toggled rest mode in compositionState, voice:', voiceIndex, 'isRest:', note.isRest);
          }
        }
      }
    }
